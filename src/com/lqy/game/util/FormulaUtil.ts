...
class FormulaUtil{
     ...
     * @param defVo 
     */
    public static realDamageValue(atkVo:BaseRoleVo,defVo:BaseRoleVo,skillKey?:string):number
    {
        var realAtk:number = atkVo.atk + atkVo.level * atkVo.upAtk;
        var realDef:number = defVo.def + defVo.level * defVo.updef;
        //暴击概率
        ...
        var sumHurt:number = skillHurt * (1-0.07*Math.log(Math.ceil(realDef/100)+0.01))*(1+doubleValue*realDoubleProport*1)*rundowValue*1;
